...
이 문서는 BrandPilot Backend Rebuild의 REST API 계약만 관리합니다. 현재 개발 진행률과 다음 작업은 `PROJECT_STATE.md`, 데이터 저장 구조는 `ERD_DESIGN.md`를 확인합니다. 구현 전에는 요청·응답과 실패 계약의 기준으로 사용하고, 구현 후에는 Controller 테스트 및 추후 OpenAPI와 일치하도록 유지합니다.
...
현재 구현은 MySQL `DATETIME(6)`과 Java `LocalDateTime`을 사용하고, 로컬 애플리케이션·JDBC 시간대를 `Asia/Seoul`로 맞춥니다. 공개 API 응답에서는 서울 시간대 오프셋 `+09:00`이 포함된 `OffsetDateTime`으로 변환합니다. `LocalDateTime.now()`를 사용하는 현재 구현은 JVM 기본 시간대도 `Asia/Seoul`이라는 전제를 가지며, UTC 저장 방식으로 변경하려면 코드와 설정 및 ERD 결정을 함께 갱신합니다.
...
- 현재 범위에는 관리자나 역할별 권한 모델이 없으므로 회원가입 요청과 User 저장 구조에 역할 필드를 두지 않습니다.
...
| `1.3.0` | 2026-07-18 | 구현과 불일치하던 회원가입 Location·역할 규칙을 정리하고 현재 Asia/Seoul 기반 시간 처리 정책을 문서화 |
