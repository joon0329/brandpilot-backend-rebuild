...
next_action: SignupService 뼈대를 만들고 이메일 중복 확인부터 구현한다
...
따라서 다음 작업은 검증된 User 영속성·DTO·비밀번호 인코더를 조합하는 SignupService를 작은 단계로 구현하는 것입니다.
...
1. SignupService 뼈대를 만들고 이메일 중복 확인을 구현한다.
2. 비밀번호 해싱, User 저장과 SignupResponse 변환을 구현한다.
3. 회원가입 Service의 정상·중복 이메일 테스트를 작성한다.
4. 회원가입 Controller와 공개 SecurityFilterChain 규칙을 구현한다.
5. HTTP 요청부터 DB 저장까지 회원가입 통합 흐름을 검증한다.
...
```text
AGENTS.md와 docs/PROJECT_STATE.md를 읽어줘. 나는 코드를 직접 작성하며 백엔드 기초와 면접 대비를 같이 공부하고 있다. 현재 체크포인트를 확인한 다음 SignupService에 완성 코드 대신 구현 순서 TODO만 준비하고, 이메일 중복 확인 TODO 하나부터 설명해줘.
```
