...
durable_checkpoint: SIGNUP_DUPLICATE_ERROR_HANDLER_TESTED
working_checkpoint: SIGNUP_DUPLICATE_ERROR_HANDLER_TESTED
next_action: SignupController 뼈대를 만들고 POST /api/v1/users를 SignupService에 연결한다
```
...
- 문서 구조 리팩터링과 SignupService 단위 테스트 체크포인트는 `origin/main`에 반영됐습니다.
- 정확한 마지막 커밋과 미커밋 파일은 세션 시작 시 `git log`와 `git status`로 확인합니다.
...
- [x] 공통 ErrorCode·ErrorResponse·FieldErrorResponse 최소 구조 작성
- [x] DuplicateEmailException을 `409 DUPLICATE_EMAIL` 응답으로 변환
- [x] 2026-07-18 `./gradlew test --tests com.brandpilot.backend.common.error.GlobalExceptionHandlerTest --no-daemon --rerun-tasks` 성공

### 다음 TODO

1. [x] 공통 오류 응답 DTO와 오류 코드의 최소 구조를 결정한다.
2. [x] `DuplicateEmailException`을 `409 DUPLICATE_EMAIL`로 변환한다.
...
| 4. 공통 기반 | `IN_PROGRESS` | V1 Flyway와 DUPLICATE_EMAIL 오류 변환 완료, 검증 오류와 독립 테스트 환경 미완료 |
...
현재 작업 트리의 공통 오류 응답 최소 구조와 `DuplicateEmailException`의 `409 DUPLICATE_EMAIL` 변환은 좁은 단위 테스트를 통과한 커밋 후보입니다.
